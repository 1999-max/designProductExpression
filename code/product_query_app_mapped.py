...
APP_CONFIG_DIR = Path(os.getenv("APPDATA", Path.home())) / "ProductExpressionQueryMapped"
APP_CONFIG_FILE = APP_CONFIG_DIR / "config.json"
MAPPING_FILENAME = "store_asin_mapping.json"
...
class StoreAsinMapping:
    def __init__(self, path):
        self.path = Path(path)
        self._shops_by_asin = None
        self._total_rows = 0
        self._fingerprint = ""
    ...
    def shops_for(self, asin):
        self._ensure_loaded()
        return list(self._shops_by_asin.get(asin.strip().upper(), []))

    def _ensure_loaded(self):
        if self._shops_by_asin is not None:
            return
        if not self.path.exists():
            raise FileNotFoundError(f"未找到 ASIN 店铺映射文件：{self.path}")
        ...
        for row in rows:
            ...
            if not asin or sid in (None, ""):
                continue
            shop_name = row.get("店铺名") or row.get("shop_name") or row.get("name") or ""
            shop = {"sid": str(sid), "name": str(shop_name)}
            shops = shops_by_asin.setdefault(asin, [])
            if not any(existing["sid"] == shop["sid"] for existing in shops):
                shops.append(shop)

        for shops in shops_by_asin.values():
            shops.sort(key=lambda item: int(item["sid"]) if item["sid"].isdigit() else item["sid"])

        self._shops_by_asin = shops_by_asin
        self._total_rows = len(rows)


ASIN_MAPPING = StoreAsinMapping(resource_path(MAPPING_FILENAME))


def format_shop_text(shops):
    parts = []
    for shop in shops:
        sid = str(shop.get("sid", "")).strip()
...
def clean_principal_text(principal_text):
    principal_text = str(principal_text or "").strip()
    return re.sub(r"(?:；\s*)?店铺[:：].*$", "", principal_text).strip()


def normalize_principal_names(value):
    names = []
    ...
    def add_one(item):
        ...
        if isinstance(item, str):
            cleaned = clean_principal_text(item)
...
def format_principal_names(value):
    return ", ".join(normalize_principal_names(value))
...
class ProductQueryService:
    ...
    def get_shops(self, asin):
        asin_key = asin.strip().upper()
        self.log(f"正在读取 ASIN 店铺映射：{ASIN_MAPPING.path}")
        shops = ASIN_MAPPING.shops_for(asin_key)
        if not shops:
            raise ValueError(f"映射表中没有找到 ASIN：{asin_key}")
        self.log(f"映射表共 {ASIN_MAPPING.total_rows} 行，ASIN {asin_key} 命中 {len(shops)} 个店铺")
        for index, shop in enumerate(shops, start=1):
            shop_label = f"{shop['sid']} {shop.get('name', '')}".strip()
            self.log(f"映射店铺 {index}/{len(shops)}: {shop_label}")
        return shops
    ...
    def query(self, asin, selected_date, mode):
        ...
        shops = self.get_shops(asin)
        self.current_shops = shops
        self.current_shop_text = format_shop_text(shops)
        ...
        for range_label, start_date, end_date in ranges:
            ...
            for index, shop in enumerate(shops, start=1):
                ...
                try:
                    ...
                    principal_text = self._records_principal_text(records)
                    if principal_text:
                        self.log(f"负责人: {principal_text}")
                    elif records:
                        self.log("负责人: 本次接口记录 principal_names 为空")
    ...
    def _records_principal_text(self, records):
        names = []
        for item in records:
            for principal in normalize_principal_names(item.get("principal_names")):
                if principal not in names:
                    names.append(principal)
        return format_principal_names(names)
    ...
    def _collect_records(self, range_data, range_label, records):
        ...
        for item in records:
            ...
            for principal in normalize_principal_names(item.get("principal_names")):
                target["principal_names"].add(principal)
    ...
    def _build_compare_rows(self, asin, range_data):
        ...
        principal_names = sorted(all_principals)
        principal_text = format_principal_names(principal_names)
        rows = []
        for metric_key, _metric_label in METRIC_ROWS:
            ...
            rows.append({
                ...
                "principal_names_text": principal_text,
                "shop_text": self.current_shop_text,
...
class ProductQueryApp(tk.Tk):
    ...
    def _build(self):
        ...
        ttk.Label(summary, textvariable=self.summary_principal_var, style="Hint.TLabel", wraplength=260).grid(row=0, column=3, sticky="w", padx=(10, 22))
        ttk.Label(summary, text="店铺", style="Field.TLabel").grid(row=0, column=4, sticky="w")
        self.summary_shop_var = tk.StringVar(value="-")
        ttk.Label(summary, textvariable=self.summary_shop_var, style="Hint.TLabel", wraplength=420).grid(row=0, column=5, sticky="ew", padx=(10, 0))
    ...
    def _load_cache(self, asin, selected_date, mode):
        ...
        if data.get("cache_schema_version") != CACHE_SCHEMA_VERSION:
            return None
        if data.get("mapping_fingerprint") != ASIN_MAPPING.fingerprint:
            return None
        rows = data.get("rows") or []
        if any(not isinstance(row.get("principal_names", []), list) for row in rows):
            return None
        if self._enrich_cached_rows(data, asin):
            with open(path, "w", encoding="utf-8") as f:
                json.dump(data, f, ensure_ascii=False, indent=2)
        return data

    def _enrich_cached_rows(self, data, asin):
        shops = ASIN_MAPPING.shops_for(asin)
        shop_text = format_shop_text(shops)
        if not shop_text:
            return False
        changed = False
        rows = data.get("rows") or []
        for row in rows:
            original = row.get("principal_names", [])
            cleaned = normalize_principal_names(original)
            if cleaned != original:
                row["principal_names"] = cleaned
                changed = True
            principal_text = format_principal_names(cleaned)
            if row.get("principal_names_text") != principal_text:
                row["principal_names_text"] = principal_text
                changed = True
            if row.get("shop_text") != shop_text:
                row["shop_text"] = shop_text
                changed = True
        return changed

    def _save_cache(self, asin, selected_date, mode, rows, failed_count):
        path = self._cache_path(asin, selected_date, mode)
        principal_names = rows[0].get("principal_names", []) if rows else []
        payload = {
            "cache_schema_version": CACHE_SCHEMA_VERSION,
            "mapping_file": MAPPING_FILENAME,
            "mapping_fingerprint": ASIN_MAPPING.fingerprint,
            "mapping_rows": ASIN_MAPPING.total_rows,
            "asin": asin,
            ...
            "principal_names": principal_names,
            "shop_text": rows[0].get("shop_text", "") if rows else "",
            "rows": rows,
        }
        with open(path, "w", encoding="utf-8") as f:
    ...
    def _finish_query(self, rows, failed_count, save_cache=True):
        ...
        if rows:
            ...
            self.summary_principal_var.set(format_principal_names(rows[0].get("principal_names", [])) or "-")
            self.summary_shop_var.set(rows[0].get("shop_text", "-") or "-")
        for row in rows:
    ...
    def _write_excel(self, path):
        ...
        for row in self.result_rows:
            values = []
            for key, _label, _width in EXPORT_COLUMNS:
                value = row.get(key, "")
                if key == "principal_names":
                    value = format_principal_names(value)
                values.append(value)
            ws.append(values)
